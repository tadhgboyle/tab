    ...
    <div class="h-full flex flex-col px-3 py-4 overflow-y-auto bg-gray-50 border-r border-gray-200">
        ...
        <div class="flex-1">
            ...
            <ul class="space-y-2 font-medium text-md">
                @if(auth()->user()->family)
                    <x-nav-link-next :routes="['family_view']" :icon="'🏠'" :name="'Family'" :url="route('family_view', auth()->user()->family)" />
                @endif

                @permission(\App\Helpers\Permission::DASHBOARD)
                    <x-nav-link-next :routes="['dashboard']" :icon="'📊'" :name="'Dashboard'" :url="route('dashboard')" />
                @endpermission

                @permission(\App\Helpers\Permission::CASHIER_CREATE)
                    <x-nav-link-next :routes="['cashier']" :icon="'🛒'" :name="'Cashier'" :url="route('cashier')" />
                @endpermission

                @permission(\App\Helpers\Permission::ORDERS)
                    <x-nav-link-next :routes="['orders_list']" :icon="'📦'" :name="'Orders'" :url="route('orders_list')" />
                @endpermission

                @permission(\App\Helpers\Permission::PRODUCTS)
                    <x-nav-link-next :routes="['products_list']" :icon="'🏷'" :name="'Products'" :url="route('products_list')" :sublinks="[
                        ['route' => 'products_ledger', 'name' => 'Ledger'],
                    ]" />
                @endpermission

                @permission(\App\Helpers\Permission::PRODUCTS)
                    <x-nav-link-next :routes="['activities_calendar']" :icon="'📅'" :name="'Activities'" :url="route('activities_calendar')" :sublinks="[
                        ['route' => 'activities_list', 'name' => 'List'],
                    ]" />
                @endpermission

                @permission(\App\Helpers\Permission::USERS)
                    <x-nav-link-next :routes="['users_list']" :icon="'👥'" :name="'Users'" :url="route('users_list')"/>
                @endpermission

                @permission(\App\Helpers\Permission::FAMILIES)
                    <x-nav-link-next :routes="['families_list']" :icon="'👪'" :name="'Families'" :url="route('families_list')"/>
                @endpermission
            </ul>
        </div>
        ...
        @permission(\App\Helpers\Permission::SETTINGS)
            <div class="mt-4 border-t border-gray-200 pt-4">
                <ul class="space-y-2 font-medium text-md">
                    <x-nav-link-next :routes="['settings']" :icon="'⚙️'" :name="'Settings'" :url="route('settings')"/>
                </ul>
            </div>
        @endpermission
